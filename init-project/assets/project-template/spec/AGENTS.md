...
spec 落在 `spec/<spec id>/`（`spec.md` + `assets/`）。没有 spec 时项目以框架形态直接运转——`spec/` 只有本文件与 `build/`（机制说明书 + 构建规则，均为常备件）。
...
3. **流程抽象为 spec**：按本目录自带的 `build/`（`build.md` 构建规则 + `spec-template/` 母版）写成 `spec.md`（头部 + §一 工作流 + §二 概览·入口·检查点 + §三 各阶段三段式），按需补 `assets/` 产出骨架。
...
- **`build/` 默认不读**：属「构建规则」（随模板出厂在本目录），只在造 / 改 spec 时才读。
