...
- **json 放脚本会读的**：字段供脚本解析（构建 / 实例化脚本读 id / name /
...
- **来源标注归锁文件**：模块是「云端实例化副本」还是「项目私有」，源头与
...
特有的约定、参数、踩坑备注等。它是实例化后项目自己的增补：云端模块不带它，
不参与锁文件指纹（改它不算漂移）。

## 四、锁文件与冷启动校验

锁文件（`spec/<id>/lockfile.json`）是 spec 包的溯源账本：记录每个模块从云端
哪个版本实例化来 + 内容指纹（hash），防止副本漂移；spec 本身只留血缘备注
...
- 一致 → 无漂移，正常干活；
- 不一致 → 有未登记的 fork / 改动，停下来提示用户，不要擅自继续。确认属
  有意改动后，四条消警出路按情形选一：① 合法升级 / 重新实例化 → 重跑
  `lockfile.py <spec>` 生成模式，登记新 version + hash；② 只是项目内补充 →
  挪进模块顶层 `add.md`（不参与指纹，改它不算漂移）；③ 模块转项目私有 →
