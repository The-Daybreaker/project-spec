...
- **`spec.md` 主干**：声明这套工作流适用于什么场景、分哪几个阶段、阶段怎么衔接、设几档入口（流程调节）、哪里设检查点，以及每个阶段的产出落点、运行过程与规范边界；阶段链是推荐推进顺序、并不强制。
- **`assets/` 骨架**：放各阶段产出文档的参考骨架，`spec.md` 按需指向它（渐进式披露：主文档精炼、用于导航，骨架作为附件按需读）。
