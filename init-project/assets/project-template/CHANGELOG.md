...
- 规范正文从 `AGENTS.md` 拆出到 `spec/constitution.md`（宪章 / 项目级 / 项目
  ...
  §五 收敛为「宪章」必读指引节（声明用户可自行修改 / 添加 / 维护 + 指向
...
- 全仓措辞规范化：内部比喻与生造术语统一换成业界通用说法（状态标记、当前
  状态区、必读指引、框架自带文件、渐进式披露等），历史 CHANGELOG 条目同步
  改写、不改变任何历史事实；不涉及机制变更。
- 规范正文从 `spec/constitution.md` 全量迁回本文件 §五「规范（用户维护区）」、
  删除该文件（决策 66）：冷启动链、§一 目录地图、§二 读盘、§六 读取约定、
...
- 锁文件三类：`spec/lockfile.py`、`spec/lockfile.md`（及实例化后的
  `lockfile.json`）——spec 拉进项目即自由编辑、无只读副本可锁，防漂移
  校验废弃；冷启动不再跑 `lockfile.py --verify`。
- 根 `package.json`：`version`（0.9.0）与本 CHANGELOG 的版本标题重复、违反单一
  事实源，且随模块制废弃已无脚本读取（原读它的 `build_delivery.py` 早已删除）；
...
- `process/memory.md`：agent 记忆固定文件（踩过的坑与被验证过的判断，
  冷启动必读；只存当前有效条目，失效归档 `logs/memory/`）。
- 云端模块库接入：锁文件 `origin` 记真实指向
  （`github.com/The-Daybreaker/project-spec`）；拉取 spec 包时记录真实来源：
  模块记 source / origin / version / hash（fork 另记 baseline），spec 只记
  source / origin / version 来源备注，不算指纹、不参与漂移校验。
- 锁文件 source 三态：模块分 cloud / fork / private（自行实现
...
- 预置 spec `spec/preset/software-dev`（8 模块）与内置构建规则
  ...
  机制三类（`AGENTS.md` 机制说明书 + `lockfile.py` 工具 + `lockfile.md`
...
- 模板 `.gitignore` 补 `__pycache__/` / `*.pyc`；AGENTS.md 两处用户维护
  区引用同步为「写给用户」；README 修正 `AGENTS.md` 大小写。
