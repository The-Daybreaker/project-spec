...
- **展示时机**：每次实质回复 / 阶段落盘 / 上下文恢复后第一条回复 / 收尾 /
  ...
  阶段线（当前节点加粗）+ 合规两行 + 反定型紧凑内容；**阶段卡内一律中文名称、
  不显示字母缩写**（格式见 `dev/PHASES.md` §5）；**每次阶段/子阶段完成：落盘
  `dev/STATUS.md` → 展示阶段卡 → git 提交（主仓库 + private 子 git，提交信息带阶段
  标识，如 `feat(P3.2): 实施子阶段 - 描述`）**；决策型推进前，阶段卡附 **📌 共识
  快照**（逐项表态，红线 17）。
