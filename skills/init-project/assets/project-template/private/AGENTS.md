...
3. **开发前阶段（M/L 档必走，S 档跳过）**：
   **可视化确认（强制，先出图再确认）**：凡改动涉及**用户界面/交互**、**系统
   架构/结构**或**业务/技术流程/状态**的，必须先产出对应图/原型——流程图
   （业务/用户流程、时序、状态）随 PRD/RFC 同目录、架构图（组件/数据流/部署）
   随 RFC/ADR 同目录、页面原型/设计稿落 `dev/prototype/`（格式 Mermaid 或 SVG
   单文件，一文件一图）→ **向用户展示并获确认后**才允许进入下一步（未确认不
   继续；纯后端无界面项目、S 档最小改动可声明跳过，并在 WORKLOG 记录理由）。
   ...
   b. **PRD**：新建 `dev/prd/PRD-XXXX-<slug>.md`（草稿）→ 涉及业务/用户流程时附
      流程图（`dev/prd/` 同目录单文件）→ 评审（图随稿展示确认）→ 用户确认定稿
      （门禁 1：字段齐全，见 `dev/prd/INDEX.md`）→ **同步更新 PRD INDEX**；
   c. **RFC**（M 复杂 / L 必走）：新建 `dev/rfc/RFC-XXXX-<slug>.md`（候选方案
      对比 + 调研引用）→ 涉及系统结构/交互时序/状态转换时附架构图或时序状态图
      （`dev/rfc/` 同目录单文件）→ 用户拍板（已采纳/已否决）→ **同步更新 RFC
      INDEX**；
   d. **ADR**（架构级）：用户确认后新建 `dev/adr/ADR-XXXX-<slug>.md`（只增不改）
      → 架构级决策必要时附图（`dev/adr/` 同目录单文件）→ **同步更新 ADR INDEX**；
      本文件「用户确认的设计决策」D-xxx 记一行摘要 + `详见 ADR-XXXX`；
   e. **DESIGN 吸收**：定稿方案覆盖式写入 `dev/DESIGN.md`（正文=当前状态，顶部
      链接 RFC/ADR，不复制历史）；涉及界面/交互时产出页面原型/设计稿
      （`dev/prototype/`，一文件一原型）并向用户展示确认。
