...
禁止直接抛方案选项**（「技术决策先解释后选择」）；提问遵守红线 17（禁面板 /
回答重检 / 共识快照 / 反定型）。
...
- **调研（红线 13）**：立项类话题先 GitHub 调研现成参考，向用户展示结果并提醒
...
- **实施（P3）**：按 DESIGN 规范修改；**同步更新受影响文档**（「改动完成即文档
  就绪」，红线 12）；对应 PRD 范围完成后 PRD 状态=已实现 + 填实现版本 + 更新 INDEX；
...
- [ ] **阶段落盘已完成**（每个阶段/子阶段后 STATUS 与受影响文档已更新，红线 14；
      阶段卡硬事实与实际一致）
