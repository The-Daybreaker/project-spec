...
- **展示时机**：每次实质回复 / 阶段落盘 / 上下文恢复后第一条回复 / 收尾 /
  用户询问进度时，必须展示**阶段卡**（纯闲聊除外）：`📍 模块 Pn · 子阶段` +
  正在完成/已完成/下一步 + 状态；**每次阶段/子阶段完成：落盘 `dev/STATUS.md` →
  展示阶段卡 → git 提交（主仓库 + private 子 git，提交信息带阶段标识，如
  `feat(P3.2): 实施子阶段 - 描述`）**；决策型推进前，阶段卡附 **📌 共识快照**
  （逐项表态，红线 17）。
