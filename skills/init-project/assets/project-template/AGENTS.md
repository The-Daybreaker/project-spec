...
4. **阶段恢复**：按 STATUS 快照「📇 阶段卡」判断当前阶段 → 按「任务影响清单 → 要读
   ...
   仍不确定则询问用户。
5. **上下文压缩后或任何新对话开始时，必须先完成以上重读（红线 15），不得凭记忆
   直接继续。**
...
- **P1 需求引导（强制）**：先复述意图、提开放问题帮用户澄清真实需求，**未澄清前禁止
  直接抛方案选项**（「技术决策先解释后选择」）；立项类话题先按红线 13 GitHub 调研并
  提醒「先调研再立项」；提问遵守红线 17（禁面板 / 回答重检 / 共识快照 / 反定型）。
...
阶段卡（模块·子阶段 / 正在完成 / 已完成 / 下一步 / 状态）+ **生命周期合规清单**
（文档同步/STATUS 落盘/git 提交/质量校验/确认/沉淀逐条勾选）+ 下一阶段输入预告；
缩写附中文翻译；以 `private/dev/STATUS.md` 为单一真相；决策型推进前，阶段卡附
**📌 共识快照**（已达成的共识逐项，用户逐项表态，红线 17）。**每次阶段/子阶段完成：
落盘 STATUS → 展示阶段卡 → git 提交（主仓库 + private 子 git，提交信息带阶段标识）。**
...
12. **文档同步与治理**：改动涉及用户视角或流程时，受影响文档在**同一次改动**内更新；
    **发现过时立即修（不论是否本轮引入），无变更时也须校准状态文档**（如 STATUS
...
13. **立项调研先行**：讨论项目思路/需求/架构/功能/产品等**立项类话题**时，
    **优先在 GitHub 调研现成参考**（相似项目、方案、库），并向用户展示调研结果、
    ...
    作为默认方案。
14. **阶段落盘**：任务过程中每完成一小阶段，先更新 `private/dev/STATUS.md` 快照与
    受影响文档再继续；进度不得只存在于对话记忆。
15. **上下文恢复重读**：上下文压缩后或任何新对话开始时，必须重读红线规范（根
    `AGENTS.md` 与 `private/AGENTS.md`）与相关文档（`STATUS.md`、`DESIGN.md`、
...
17. **提问与共识确认（禁面板 / 回答≠确认 / 共识快照 / 反定型）**：禁止调用问题
    面板/选择面板类结构化 UI 向用户提问（含 request_user_input / AskUserQuestionTool
    类工具）；需要提问时只在聊天/文本中展示开放问题并中止对话等待用户输入。聊天内
    可给出选项与推荐（信息辅助），但不得仅依赖选择项盲目向下推进，确认必须以用户
    自由文本的明确表态为准。用户回答 ≠ 需求确认：每次回答后作为新信息输入，重检
    问题空间（哪些问题失效、哪些新问题浮现、哪些假设被推翻/加强）。关键/风险节点
    展示反定型 6 项（当前理解 / 我的假设 / 尚未确认 / 反例 / 替代解释 / 重新审视
    问题），平时每次提问后内部思考。每次推进前展示共识快照、用户逐项表态；达成
    共识 = 反定型完整展示（关键节点）+ 用户明确确认，此过程不得 push 流程、不得
    诱导。确认不锁定：需求阶段内任何新信息可回到该流程重审；PRD 定稿后变更走
    变更流程（开新 PRD 取代）。
