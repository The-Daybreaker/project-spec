...
| 自动发布（`--auto-release`） | 否 | 关闭 | 开启「每次改动完成后自动发布」（视为
  发布/推送预授权，红线 3 对常规发布视为已满足；破坏性变更、永久删除仍须单独确认）；
  默认不自动发布（用户确认后执行发布流程） |
...
- [ ] `docs/LOADING.md` 已生成（v1.4.0 特性：加载规则表；流程图随
      `private/dev/PHASES.md`；面向用户的指南在 `README.md`）
...
| Python 不可用 | 模板脚本需要 Python 3.9+（仅标准库），`--no-git` 并不能绕过脚本；
  提示用户安装 Python；无法安装时只能人工复制模板文件并手工替换占位符（或用其他
  机器初始化后复制） |
...
参考并提醒「先调研再立项」（模板 `AGENTS.md` 红线 14），避免从零造轮子。
