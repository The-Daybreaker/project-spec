...
2. **先对齐后实施**：实施之前必须对齐需求和计划，得到用户确认之后再实施；
   未确认不实施（低风险例外见第 2 条）。
3. **变更分级**：高风险操作（大规模影响文件、破坏性变更、永久删除、发布/推送、
   修改用户数据）先展示方案、征得用户同意后执行；低风险先执行、随后记录、告知用户
   并给出回退方案。
4. **实施后自动审计**：每次实施完成后按「自动审计」执行，优先独立子 agent 审计。
5. **删除纪律**：对话内删除先移动到项目本地 `_trash/<agent产品名>_<YYYY-MM-DD>_<HHMM>/`
...
13. **文档同步与治理**：改动涉及用户视角或流程时，受影响文档在**同一次改动**内更新；
...
14. **立项调研先行**：讨论项目思路/需求/架构/功能/产品等**立项类话题**时，
    **优先在 GitHub 调研现成参考**（相似项目、方案、库），并向用户展示调研结果、
    提醒用户**「先调研再立项」**；未经调研不引导用户立项，不把「从零造轮子」
    作为默认方案。
15. **阶段落盘**：任务过程中每完成一小阶段，先更新 `private/dev/STATUS.md` 快照与
    受影响文档再继续；进度不得只存在于对话记忆。
16. **上下文恢复重读**：上下文压缩后或任何新对话开始时，必须重读红线规范（根
