...
紧凑内容；格式见 `private/dev/PHASES.md` §5）；**阶段卡内一律使用中文名称，不显示
字母缩写**（如 需求阶段/方案文档/架构决策记录/状态快照）；以 `private/dev/STATUS.md`
为单一真相；决策型推进前，阶段卡附 **📌 共识快照**（已达成的共识逐项，用户逐项
表态，红线 17）。**每次阶段/子阶段完成：落盘 STATUS → 展示阶段卡 → git 提交
