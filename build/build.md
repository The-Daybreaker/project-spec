...
> 各模块的 `MODULE.md`。
>
> 需造 / 改 spec 或模块时，按模板地基 `spec/AGENTS.md` 的指引拉取本目录到
...
（`spec/lockfile.md` 讲字段，`spec/lockfile.py` 生成校验，均在模板地基
...
- **spec 包形态**：一个 spec 包 = 组织清单（`manifest.json`）+ 入口指引
  ...
  空值等**机制通则的单一来源在模板地基 `spec/AGENTS.md`**（出厂自带），
  spec 包的 `AGENTS.md` 只写本包运转（这是什么 + 依赖全景 + 入口判定）。
- **入口缩放**：一套 spec 该定义**多个入口**，不同等级的任务从不同入口进、
  启用不同模块子集——简单的事不背重流程，复杂的事不漏环节。入口定义（启用
  哪些模块）在 `manifest.json` 的 `entries`（集合、不表顺序，执行顺序由依赖
  图决定）；入口判定语义在 spec 的 `AGENTS.md`；模块间依赖用一张 mermaid
  全景图表达（放 spec 的 `AGENTS.md`）。
...
**不包含**：锁文件工具与字段规范（家在地基 `spec/lockfile.py` / `lockfile.md`，
随模板出厂）。
