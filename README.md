...
- **`preset-spec/`——spec 最佳实践集**：预置的工作流最佳实践，供你参考。做自己的 spec 为主，需要时也可以从这里复制一份进项目自由修改，也欢迎回流你设计的 spec。
...
```mermaid
flowchart TD
  S["新会话开始 → 读盘"] --> A["AGENTS.md<br/>协作总纲 · §四 规范（用户维护）"]
  A --> G["spec/AGENTS.md<br/>spec 机制说明书（短件常备）"]
  G --> P["process/ 全部文件<br/>任务板 · AI 记忆 · 临时件"]
  P --> Q{"装了工作流 spec？"}
  Q -- 是 --> F["spec/&lt;id&gt;/spec.md：判定入口、走阶段"]
  Q -- 否 --> W["按框架直接干活"]
  F --> W
...
flowchart TB
  ...
  subgraph PROJ["初始化出的新项目"]
    ...
    FRAME["框架层：协作总纲 · 任务板 · AI 记忆 · 归档 · git 约定 · 构建规则<br/>每个项目相同 · 出厂自带"]
    SPEC -. "叠加" .-> FRAME
  end
