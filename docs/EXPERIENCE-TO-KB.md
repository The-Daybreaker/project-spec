...
- 最后更新：2026-08-27 23:40

## 索引

| 日期 | 标题 | 类型 | 状态 |
|---|---|---|---|
| 2026-08-27 | 扫描器/冒烟自测三坑：占位示例 `git@github.com:user/…` 会二次匹配 `user@github.com`（白名单按完整示例加 `@github\.com`）；冒烟注入的假 token 字面量会被同一扫描器当高危（运行时拼接，注入文件仍可拦截）；冒烟默认作者名会触发个人信息（显式 `--author` 测试名） | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | P2 收口实证：RFC 逐点表态全部按推荐 → 评审记录写入文档 → ADR-0002 只增不改（9 决策 + 7 不选）→ 母项目沿用轻量流程（不建 DESIGN 直接实现） | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 方案阶段决策点化（RFC 候选对比 D1-D7，每点给 A/B/C + 推荐 + 理由）：把大改造拆成可逐点表态的决策，用户按点确认，避免整篇方案「整体通过/整体推翻」；评审记录写入需求文档（可选增强不采纳） | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 第二轮调研对齐：需求分层（愿景→路线图→版本需求）与 F8 一致、路线图须为单一事实来源；UAT/IEEE 829 用例要素（编号/场景/预期/负责人/签署）；共识卡与 premortem/信心等级同构；三要素验收面=政策即代码轻量落地（可执行检查优先+人工清单兜底） | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 需求文档分层：长期需求/展望/排期文档（路线图，唯一长期入口）+ 每版本需求文档（本版本增量），实现「每版本只看长期文档+本版本文档」；配套校验（版本列表与索引/版本号对齐、关联字段引用、定稿必登记） | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 推送前门禁调研结论：持续集成兜底对密钥太晚、`core.hooksPath` 是版本化钩子标准做法、gitleaks 等第三方工具与模板零依赖约束的关系（自研保留+可选增强）、三要素与需求工程「可验证」标准一致 | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 元规范设为最顶层红线 1 的编号冲击（全链引用顺延）+ 私有区需「禁远端/禁打包+例外确认」+ 按三要素体检存量规范/红线 + 辩护性措辞扫描正则可行但有误报边界（提交信息硬拦、注释/文档人工复核） | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 对 AI 的要求必须「三要素化」（意图/展示/验收）：不可展示、不可验收的要求 AI 倾向不做；红线 16 与反定型同病（无法验证是否被遵守）；番茄炒蛋/东坡肉比喻为红线 16 原始意图；反定型 6 项与共识卡冗余可合并为 4 项 | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 需求核查先做「措辞定位 + 分层现状」：用户术语与仓库措辞不一致用近义词 Grep（过度修正/留疤=红线16、共识卡=共识快照）；「有内容扫描器 ≠ 有推送门禁 ≠ 已进模板」三层分开核查 | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 敏感信息删除后全量复核：冒烟脚本 GBK 解码缺陷（子进程 UTF-8 × 父进程 GBK）+ 技能摘要级红线条数过时（自动化校验不覆盖）+ 六处副本重装验证 | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 未发版区段补漏按时序插入（非一律置顶）+ 全局/本地两级身份切换后须回归核对新提交作者 + 审计报告的「审计后进展」补注模式 | 经验方法/教训 | 待沉淀 |
...
## 2026-08-27 · 扫描器与冒烟自测三坑（P4 自审修复实录）

- 来源项目/任务：通用项目模板工作区 PRD-0002 P4 审计验证（2026-08-27）。
- 关键方法/教训：
  1. **占位示例的二次匹配**：`git@github.com:user/my-app.git` 这类示例中，邮箱
     正则先匹配 `git@github.com`（已在白名单），继续扫描还会匹配
     `user@github.com`（不在白名单）→ 误报个人信息。白名单要覆盖「示例的
     全部可匹配片段」，直接加 `@github\.com` 域级白名单。
  2. **自测注入物会被同一扫描器误判**：冒烟脚本里写死的假 token 字面量
     `ghp_FAKE...` 会被 `scan_secrets` 当高危凭据（扫描器只排除自身源码）。
     解决=运行时拼接（`"ghp_" + "FAKE..."`），源码不连续不命中，注入文件
     内容仍是完整假 token、可正常验证拦截。
  3. **冒烟默认作者名=真实个人信息**：初始化默认取 git 全局 `user.name`
     （如默认用户名）写入 LICENSE，`--strict` 即红。冒烟应显式
     `--author smoke-user`；真实项目 LICENSE 作者名属用户真实信息，扫描器
     人工复核语义正确，不应全局白名单。
  4. **子智能体停用后自审补位**：用户指示 Codex 内不用子智能体后，独立审计
     改为自审（逐项留证 + 全链脚本复跑 + 测试报告双区记录），发现 4 项全部
     修复并复扫确认。

## 2026-08-27 · 方案阶段收口：RFC 全部按推荐 → ADR-0002 → 轻量流程进入实现

- 来源项目/任务：通用项目模板工作区 PRD-0002 → RFC-0002 → ADR-0002
  （2026-08-27，大功能升级档位 P1+P2 走完）。
- 关键方法/方案：
  1. **逐点表态的收口方式**：用户「全部按推荐」即完成七个决策点的评审；评审
     记录写入 RFC（状态=已采纳 + 评审记录段），可追溯。
  2. **ADR 只增不改**：ADR-0002 记录 9 项决策（红线 1/19、门禁三层、私有区防
     外发、测试模块、措辞扫描、共识卡、需求分层、轻量流程）与 7 项不选理由；
     作为开发阶段的实施依据，后续只增不改。
  3. **母项目轻量流程沿用**：ADR-0001 先例「不建 DESIGN 文件，ADR 确认后直接
     进入实现」对本模块继续有效；模板骨架的 DESIGN/登记册保持不动。

## 2026-08-27 · 方案文档决策点化：RFC 候选对比让用户逐点表态

- 来源项目/任务：通用项目模板工作区 PRD-0002 → RFC-0002（2026-08-27，
  需求阶段收口进入方案阶段）。
- 关键方法/方案：
  1. **大改造拆成决策点**：RFC-0002 把实施层问题整理为 D1-D7 七个决策点，每点
     列候选（A/B/C）+ 推荐 + 理由（如辩护性措辞扫描独立脚本 vs 并入扫描器；
     钩子全量扫 vs 只扫 diff；精简版规范吸收 vs 新增条目；合规七项 vs 独立块；
     路线图/共识卡校验强度）。用户逐点表态，避免「整体通过/整体推翻」。
  2. **评审记录写入文档**：PRD-0002 定稿时把「可选增强不采纳」作为评审记录
     写入正文（F7），决策可追溯；与「决策修改覆盖正文 + 变更日志一行」的文档
     治理并行不冲突（评审记录属定稿状态的一部分）。
  3. **确认不锁定闭环实证**：评审后用户追加 F8 → 回审 → 定稿，完整走通
     「需求阶段内新信息可回审，定稿后走变更流程」。

## 2026-08-27 · 第二轮调研：需求分层 / UAT / premortem / 政策即代码（PRD-0002）

- 来源项目/任务：通用项目模板工作区 PRD-0002 第二轮调研（2026-08-27，用户要求
  「本轮需求再做一次调研」）。
- 关键方法/结论：
  1. **需求分层是业界标准**：愿景 → 路线图 → 版本/发布 → 需求；路线图必须是
     持续更新的单一事实来源（GitLab/Atlassian 强调 honest & strategic）。F8
     双层结构（长期路线图 + 版本需求文档）与之对齐。
  2. **UAT/IEEE 829 文档要素**：用例编号/场景/步骤与数据/预期/负责人/签署标准
     ——测试报告双区（自测区 + 用户验收区）按此补全，用户验收不再只是「人工
     验证清单」一句话。
  3. **决策防偏有成熟实践**：premortem（Klein）、WRAP 框架、「结构性护栏」
     （检查清单 + premortem + 显式信心等级）——共识卡 4 项与之一致；可选增强
     是推进结论附「信心等级/残余风险」。
  4. **政策即代码支持「红线可验证」**：规则写成可执行、可版本化、可验证的
     产物；LLM 可把自然语言政策编译为可运行检查——三要素「验收面=可执行检查
     优先 + 人工核对清单兜底」有外部依据，模板自研扫描器即轻量版。

## 2026-08-27 · 需求文档分层：长期路线图 + 每版本需求文档

- 来源项目/任务：通用项目模板工作区 PRD-0002 需求（2026-08-27，F8 增补）。
- 关键方法/方案：
  1. **问题**：需求文档按需求/版本各一文件，无长期规划/排期载体；新对话或
     上下文恢复时要读多份历史需求文档才了解全局，阅读负担重。
  2. **方案**：双层结构——长期需求与展望文档（路线图，唯一长期入口，正文=当前
     状态覆盖式更新）承载愿景/长期目标/需求地图/版本排期；版本需求文档保持冻结
     留史，只写本版本增量，头部挂「所属版本」「关联路线图条目」字段。每版本
     只读「路线图 + 本版本需求文档」两份。
  3. **配套校验**：路线图版本列表与需求索引/version.json 对齐、需求文档关联
     字段引用存在、定稿需求必登记路线图条目——纳入 check_dev_docs 断言，避免
     路线图与登记册漂移（与「状态快照=单一真相」同构）。

## 2026-08-27 · 推送前安全门禁与要求可验证性调研（PRD-0002 前置）

- 来源项目/任务：通用项目模板工作区 PRD-0002「红线与要求可验证性改造」调研
  （2026-08-27，红线 13 先调研再立项）。
- 关键方法/结论：
  1. **持续集成兜底对密钥泄漏太晚**：一旦 push，密钥已进远端历史；本地
     pre-push 门禁是必要防线（DevOps.com、zwischen 设计文档等多来源一致）；
     服务器端 pre-receive 最强但 GitHub 普通仓库不可用。
  2. **版本化钩子 = `core.hooksPath`**：钩子不随 clone 分发；`.githooks/` 目录
     + 初始化脚本配置 `core.hooksPath` 是标准做法（git template/符号链接为备选）。
  3. **扫描工具生态**：gitleaks/trufflehog/pre-commit 框架成熟，但多为第三方
     依赖；模板零依赖约束下保留自研 stdlib 扫描器，gitleaks 等仅文档推荐
     （可选增强），避免模板引入运行时依赖。
  4. **三要素与需求工程标准一致**：需求工程要求需求 correct/unambiguous/
     verifiable/traceable；DoD 要求验收标准达成 + 证据 + 文档更新——「意图/
     展示/验收」是把标准显式化到 agent 协作，非自创怪规则。

## 2026-08-27 · 元规范定为最顶层红线 + 私有区禁远端/禁打包 + 存量规范三要素体检

- 来源项目/任务：通用项目模板工作区「推送前校验 + 红线细化 + 测试模块 + 反定型
  强化」需求讨论（2026-08-27，P1 需求引导与澄清，第二轮批注）。
- 关键方法/教训：
  1. **元规范放最顶层**：用户指定「要求三要素」为红线 1（最顶层红线）——所有
     红线顺延重编号（原 1-17 → 2-18）。重编号是全局冲击：根/私有 AGENTS、阶段
     定义、流程、审计清单、决策记录（ADR-0001 引「红线 17」→ 18）、精简版规范
     1-17 → 2-18、继承矩阵指纹、技能摘要「红线条数 17 条」全部要 grep 校准；
     审计清单「红线条数硬事实」同步更新。发版/实施时必须全局 grep 新旧编号。
  2. **私有区需要主动防泄漏机制**：仅「不提交密钥」不够——还须**禁给私有区设
     远端、禁打包**；例外（用户明确要求）要走「全面个人信息检查 → 报告 → 用户
     确认」三连，防止私有区内容无意中被外发。
  3. **存量规范也要体检**：新元规范落地后，不能只约束未来——需按三要素（意图/
     展示/验收）逐条体检现有规范与红线，产出符合性矩阵与缺口清单，再决定修复。
  4. **辩护性措辞扫描的可行性边界**：正则可抓明显模式（「已移除/不需要/为什么
     不需要/不适用」等），但误报存在（合法语境如「本检查项不适用于文档类改动」）。
     建议双档：提交信息/PR 描述硬拦（可控面小），注释/文档命中列入人工复核
     （与扫描器「高危硬拦/个人信息人工复核」同构）。

## 2026-08-27 · 对 AI 的要求必须「三要素化」：意图 / 展示 / 验收（元规范）

- 来源项目/任务：通用项目模板工作区「推送前校验 + 红线细化 + 测试模块 + 反定型
  强化」需求讨论（2026-08-27，P1 需求引导与澄清）。
- 关键方法/教训：
  1. **不可展示、不可验收的要求，AI 倾向不做**：用户实测红线 16（范围克制与
     纠错清零）和反定型都「无法验证、无法确认是否被遵守」——问题不在规则写得
     好不好，而在规则没有「展示面」（执行情况看得见）和「验收面」（结果可核对）。
     经验结论：所有对 AI 的要求都应做成可展示 + 可验收的结果，并解释原始意图。
  2. **原始意图 = 防误解的锚点**：红线 16 的原始意图是「番茄炒蛋/东坡肉」比喻——
     让做番茄炒蛋，agent 擅自加东坡肉；指出后去掉，但 PR 写「番茄炒蛋（无东坡
     肉）」、注释解释为什么不需要东坡肉。意图写进规范，agent 才知道「为什么」，
     而不只是「不做什么」。
  3. **展示面示例**：阶段卡/合规清单展示执行情况（反定型块、共识卡、重检行、
     diff 范围清单、扫描门禁结果、红线遵循核对表）——可见即监督。
  4. **验收面示例**：能自测的自测（脚本断言/扫描门禁/文档一致性），不能自测的
     交给用户验收（测试报告用户验收区、交付汇报逐项表态）。
  5. **反定型 6 项与共识卡冗余**：假设与共识可合并，其余各项可归并——内容级
     融合而非格式拼合，避免双重展示。

## 2026-08-27 · 需求核查先定位措辞、再分层核实现状（推送校验/红线定位示例）

- 来源项目/任务：通用项目模板工作区「推送前校验 + 红线细化 + 测试模块 + 反定型
  强化」需求（讨论阶段，2026-08-27）。
- 关键方法/教训：
  1. **用户术语 ≠ 仓库措辞**：用户说「防止过度修正/修正留疤」「共识卡」，仓库
     实际措辞是红线 16「范围克制与纠错清零（按单办事、不加菜；撤菜不解释）」与
     「共识快照」。直接搜关键词零命中时，应先用近义词/主题词（修正、疤痕、共识、
     重检、反定型、推倒、反复…）Grep 定位，再读上下文确认对应关系，不要急着说
     「不存在」。
  2. **「已实现」要分三层核查**：内容扫描器存在（工作区 `scan_secrets.py`，
     `--check`/`--strict`/`--history`）≠ 已接推送门禁（`.git/hooks` 只有样例、
     无 `core.hooksPath`）≠ 已进模板（`project-template/scripts/` 无此脚本，
     `pre_release_check.py` 只做文件名/路径级扫描）。需求满足度按三层分别陈述，
     缺口才能定位准确。
  3. **反定型约束力弱的机制根因可枚举**：关键/风险节点定义模糊（有裁量空间）、
     反定型是条件块（平时不出现、无留档）、内部思考不可监督、合规清单六项不含
     反定型。为后续「硬清单 + 固定块/共识卡融合 + 重检行外部化 + 脚本锚点」的
     方案讨论提供了证据链。

## 2026-08-27 · 全面审计（第十一轮）：敏感删除后全量复核的三大教训

- 来源项目/任务：通用项目模板工作区全面审计（用户删除仓库敏感信息后要求不依赖
  git 全量复核）。
- 关键方法/教训：
  1. **subprocess 解码坑**：中文 Windows 下 `subprocess.run(..., text=True)` 按
     系统 ANSI（GBK）解码子进程输出；子进程设 `PYTHONUTF8=1` 输出 UTF-8 时父进程
     解码必炸。只重配父进程 `stdout/stderr` 不够，必须给 `subprocess.run` 显式传
     `encoding="utf-8", errors="replace"`（或父进程也设 `PYTHONUTF8`）。冒烟自检
     由此照文档首跑必红。
  2. **摘要级硬事实自动化校验覆盖不到**：sync 只校验版本/指纹/文件级覆盖，
     摘要把关靠人工——init-project SKILL.md「红线 16 条」在模板新增红线 17 后
     漏更新，正是此类。
  3. **安装副本漂移机制有效**：改 SKILL.md 后 sync 立即拦截六处副本不一致，
     重装 + `verify_installed_copies` 全绿恢复。
  4. **不依赖 git 的全量复核方法**：全文件内容扫描（路径/用户名/邮箱/凭据）+
     脚本实跑 + 镜像哈希 + 版本字面 grep + 链接/编码检查，可作为敏感删除后的
     标准复核流程。
- 相关文件：`scripts/smoke_init.py`、`skills/init-project/SKILL.md`、
  `install-targets.json`、`docs/LOADING.md`、`docs/AUDIT-2026-08-27-r11.md`

## 2026-08-27 · 审计遗留处置三细节：补漏时序 / 两级身份回归 / 审计后进展补注

- 来源项目/任务：通用项目模板工作区第十轮审计遗留处置（P2-1 漏登补录、
  P3-1 措辞校准、全局身份切 noreply）。
- 关键方法/教训：
  1. **补漏按时序插入，不是一律置顶**：「新条目在前」指**新增当时**的条目；
     事后补登历史漏项时，插入位置按事件时间（`git log` 时序）放——本轮 5 条
     漏登插在最新安全条目与最旧 origin 条目之间，而非压过新条目。
  2. **身份切换要查两级**：仓库本地 `user.email` 与全局配置相互独立，只改一处
     另一处仍会污染新提交（本轮全局改后回归核对新提交作者均为 noreply）。
  3. **审计报告的处置留痕模式**：发现小节末尾加「审计后进展（同日）」一行 +
     §六处置状态统一收口——报告正文保持「审计时点」原貌，处置动态只追加不重写。
- 关联产物：`docs/AUDIT-2026-08-27-r10.md`（§四进展补注 + §六）。
