...
| 日期 | 标题 | 类型 | 状态 |
|---|---|---|---|
| 2026-08-27 | 作者身份匿名化全流程：gh api 取数字 ID 拼 noreply + filter-branch env-filter 元数据重写（快、不碰文件内容）+ 物理清除 + 防文档字面再泄露 + 扫描器 noreply 白名单 | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 公开仓库安全审计三盲区：扫描器只覆盖 blob 不覆盖提交元数据与远端引用状态；「网络不可达」类声称会随时间失效须现场复测；未发版区段在贯穿任务期易漏登 | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 公开仓库历史泄露处置全流程：filter-branch 重写 + 旧对象物理清除 + 全历史复扫 + force push；清洗脚本会误伤扫描器自身模式需自排除 | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 公开仓库安全扫描方法：当前文件 + 全历史 blob 双扫（0 高危凭据、发现本机路径/用户名残留）；正则类误报要先核实再定性 | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 沙箱遮蔽 `gh` 系统钥匙串令牌：沙箱内 `gh auth status` 误报无效、hosts.yml 无 token 均可能是沙箱视图；排查应先提权重查，别急着让用户重登 | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 公开文档去「具体项目来源」要全局 sweep：引言 + 专门来源节 + 本地路径引用一并清理，并通俗化面向用户 | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | `gh` CLI 令牌与 git push 凭据相互独立：push 成功 ≠ Release 可建；发布前先 `gh auth status`，失效即让用户重登 | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 沙箱只读 `.git` 下配置远端需升级权限；维护/贯穿类任务同样走「STATUS 双收口 + 自动沉淀」纪律 | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 格式类红线的「断言面/表述面」失同步根因与机制化杜绝：冒烟深度代码化（smoke_init）+ 断言面联动核对清单 + 修复轮 sweep 比审计多发现 6 处的实证 | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | 开箱即用审计要穿透到「骨架脚本自检」：第九轮全链实测发现新项目首跑即红（字面断言与合并阶段卡骨架失同步） | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 阶段卡反定型为条件块（仅关键/风险节点展示；收尾/进度节点不带）+ 母项目本地发版 v1.4.1（无远端：版本同步+tag） | 经验方法/教训 | 待沉淀 |
| 2026-08-27 | 阶段卡/生命周期合规清单/反定型合并紧凑模块：横置阶段线 + 合规两行 + 反定型紧凑内容 + 全中文展示（降低理解成本） | 经验方法/方案 | 待沉淀 |
| 2026-08-27 | agent 提问与共识确认机制（红线 17）：回答≠确认、禁面板、共识快照+逐项表态、反定型 6 项关键节点展示（组合创新，无同款） | 经验方法/方案 | 待沉淀 |
...
## 2026-08-27 · 作者身份匿名化：noreply 重写 + 物理清除 + 防再泄露（历史遗留解决）

- 来源项目/任务：通用项目模板工作区历史遗留解决（第十轮审计 P3-2，用户选择
  重写为 noreply 并覆盖强推远端）。
- 背景与上下文：全部 82 提交作者为真实邮箱，公开仓库全网可见；用户确认重写为
  GitHub noreply 邮箱，与远端强推一次成型。
- 关键方法/经验：
  1. **noreply 邮箱拼装**：格式 `<数字ID>+<账号名>@users.noreply.github.com`；
     数字 ID 用 `gh api users/<账号名> --jq .id` 获取，不要猜。
  2. **仅元数据重写用 env-filter**：`git filter-branch -f --env-filter`（只替换
     GIT_AUTHOR_EMAIL / GIT_COMMITTER_EMAIL）+ `--tag-name-filter cat -- --all`
     一次重写全部提交与 tag；不触碰文件内容（与 tree-filter 的本质区别），
     82 提交秒级完成、无清洗副作用。
  3. **物理清除同三件套**：删 `refs/original/*` + `reflog expire --expire=now
     --all` + `gc --prune=now`；验收 = `fsck --unreachable` 空 +
     `git log --all --format` 身份唯一。
  4. **新盲区——文档字面再泄露**：审计报告曾把真实邮箱原文写进证据表；元数据
     清除后「跟踪文件里仍写着邮箱」就是再泄露。身份清除后必须全局 grep 旧邮箱
     字面并一并清除（本轮命中两处，已清）。
  5. **扫描器联动**：noreply 邮箱会命中扫描器 EMAIL 模式——`scan_secrets.py`
     白名单加入 `@users.noreply.github.com`（公开账号标识，非个人信息），否则
     `--strict` 门禁误报。
  6. **重写前必备份**：`git bundle create <路径> --all` 恢复点（本轮 438 KB，
     存工作区临时目录，远端核实后可弃）。
  7. **重写后再污染盲区**：本地/全局 git `user.email` 不随历史重写改变——重写
     之后新增的提交会退回旧身份。重写后必须立即 `git config [--local]
     user.email` 指向新身份，并把「当时提交的元数据」纳入验收（本轮 3 个新提交
     被再污染，二次局部重写 HEAD~3..HEAD 修复）。
  8. **内容层与元数据层是两个独立清除面**：身份/元数据重写（env-filter）不改
     文件内容——脱敏**之前**提交的文件旧版本仍把字面留在历史 blob 里（本轮审计
     报告旧版本含邮箱原文）。清除字面后必须 `--history` 门禁复扫兜底，命中即对
     「含泄露文件的提交范围」做局部内容重写（`--tree-filter` 逐树替换）；该门禁
     本轮真实拦下残留，机制有效。
- 关联产物：`docs/AUDIT-2026-08-27-r10.md` §四 P3-2（含审计后进展补注）。

## 2026-08-27 · 公开仓库安全审计三盲区（第十轮）：扫描器覆盖模型 / 状态声称失效 / 未发版区段漏登

- 来源项目/任务：通用项目模板工作区第十轮全面审计（安全处置收尾后的复审）。
- 背景与上下文：上轮已完成历史重写 + 旧对象物理清除 + 全历史复扫全绿；本轮
  复审确认本地面确实全绿，但发现远端仍保留重写前泄露历史（P1）与多处文档漂移。
- 关键方法/教训：
  1. **扫描器覆盖模型**：内容扫描器（scan_secrets 类）只覆盖「跟踪文件 + 全历史
     blob」，至少不覆盖 ① 提交元数据（作者名/邮箱/message）② 远端引用状态
     ③ 未跟踪文件。安全结论必须写明覆盖模型：① 用 `git log --format` 专项取证、
     ③ 用 `git status` / ls-files 核对、② 把远端对照（`ls-remote` 本地 vs 远端
     SHA）列为审计必查项——**「本地清理完成」≠「公开面清理完成」**。
  2. **状态声称的时间漂移**：「网络不可达」「区段为空」类声称随时间失效；收尾/
     汇报前逐条现场复测（网络用 ls-remote / gh api，区段内容用 git log / diff），
     不沿用先前结论。本轮正是复测发现网络已恢复、待办强推可立即执行。
  3. **未发版区段漏登**：非发版期多提交的贯穿任务（安全处置）极易漏登未发版
     区段——本轮 tag 后 9 提交仅登记 1 条；收尾必须 `git log <tag>..HEAD`
     逐条比对（维护约定 #8 应作为审计必查项）。
  4. **遏制有利条件取证**：泄露事件先查 forks 数与 Release 资产面（`gh api`）——
     本轮 forks=0、无 description、无资产，大幅简化处置决策。
- 关联产物：`docs/AUDIT-2026-08-27-r10.md`。

## 2026-08-27 · 公开仓库历史泄露处置全流程：filter-branch 重写 + 物理清除 + 复扫 + force push

- 来源项目/任务：通用项目模板工作区（GitHub 公开仓库个人信息历史泄露处置）。
- 背景与上下文：扫描发现本机路径/用户名残留（当前 4 文件 + 历史旧 README 安装表
  与 EXP-KB 多版本）。用户确认执行三件套：当前清理 + 历史重写 + 防再发。
- 关键方法/经验：
  1. **先备份再重写**：`git bundle create <路径> --all` 全量备份（453 KB），
     重写不可逆操作前必须留恢复点。
  2. **重写工具选择**：`git-filter-repo` 未安装且网络不稳，用 git 内置
     `filter-branch --tree-filter <python脚本> --tag-name-filter cat -- --all`
     离线完成（80 提交 + 14 tag，约 75 秒）；脚本对每个 checkout 的树做
     长路径优先的逐字替换 + 正则替换，UTF-8 读写、跳过二进制。
  3. **物理清除旧对象**：重写后必须 `update-ref -d refs/original/*` +
     `reflog expire --expire=now --all` + `gc --prune=now`，否则旧泄露 blob
     仍随对象库存在；之后 `git fsck --unreachable` 复核为空。
  4. **清洗副作用**：scrub 会误伤「仓库内扫描器自身源码」——把 scan_secrets.py
     正则里的用户名模式替换成占位符，导致扫描器失效。处置：扫描器须**自排除**
     （跳过自身文件），并把「清洗前后各跑一次扫描器」列为重写验收步骤。
  5. **验证闭环**：重写后 `scan_secrets.py --history --check` 全绿（0 高危 /
     0 个人信息；仅剩通用占位符与示例被忽略）才算完成；force push main + tag，
     `ls-remote` 核实。
  6. **网络故障预案**：github.com:443 与 api.github.com 可用性可能分离（本次
     git push 失败但 gh api 正常）；推送受阻时先做连通性测试，不反复空等。
- 沉淀建议：可并入「公开仓库安全治理」经验条目（扫描 + 处置 + 防再发闭环）。

## 2026-08-27 · 公开仓库安全扫描方法：当前文件 + 全历史双扫，先核实再定性

- 来源项目/任务：通用项目模板工作区（GitHub 公开仓库个人信息/高危凭据排查）。
- 背景与上下文：用户要求确认上传到 GitHub 的内容里有无个人信息与 API key 等高危
  信息。仓库为 PUBLIC，任何泄露即全网可见。
- 关键方法/经验：
  1. **扫描对象 = 当前 HEAD + 全部历史**：仅扫工作树会漏掉「已删除但仍留在 git
     历史」的内容。用 `git rev-list --objects --all` 枚举全部唯一 blob，对每个
     blob 跑同一套正则（历史对象 1117 个，几分钟可完成），并用
     `git log --find-object=<sha>` 回溯涉及提交，区分「当前文件」与「仅历史残留」。
  2. **模式分两级**：高危凭据格式（ghp_/sk-/AKIA/私钥块/JWT/密钥赋值等）几乎
     零误报；个人信息（本机路径、用户名、邮箱、手机号、身份证）误报率高，命中
     后必须逐条人工核实——本次脚本的正则把 Python 字符串里的 `n:\s*` 误判为
     Windows 路径，属典型误报。
  3. **公开仓库的披露面**：本机绝对路径、系统用户名、第三方工具计划文件路径等
     属「个人信息」范畴，虽然不一定是密钥，但对公开仓库仍应清理；安装表类配置
     用 `~` 占位符（本项目 install-targets.json 即安全）。
  4. **处置分两层**：当前文件可直接清理；历史残留需 git 历史重写 + force push
     （破坏性，必须用户确认；且 GitHub 缓存/分叉仍可能残留）。
- 沉淀建议：可作为「发版前安全扫描」经验条目（与发布流程前置检查合并）。

## 2026-08-27 · 沙箱遮蔽 `gh` 系统钥匙串令牌：先提权重查，别急着让用户重登

- 来源项目/任务：通用项目模板工作区（v1.4.2.patch0 远端发布 + Release 创建）。
- 背景与上下文：沙箱内 `gh auth status` 报「token in default is invalid」、
  `gh auth token` 取不到、`hosts.yml` 无 `oauth_token`，误判为令牌过期并引导用户
  重登；用户指出「不是令牌问题，是沙箱限制，提权试试」。提权后
  `gh auth status` 显示 `✓ Logged in (keyring)`，令牌在系统钥匙串里一直有效，
  `gh release create` 一次成功。
- 关键方法/经验：
  1. **凭据类工具先提权重查再下结论**：`gh` 令牌默认存系统钥匙串（keyring），
     受限沙箱读不到钥匙串时会把「读不到」误报为「令牌无效」；`hosts.yml` 只存
     账号元数据、不一定含 token。排查顺序：先 `require_escalated` 重跑
     `gh auth status`，确认无效再让用户重登，避免无谓打断。
  2. **用户对环境的判断值得先采纳**：用户明确说「是你被沙箱限制了，提权尝试」
     时，应直接照做验证，而不是坚持「令牌失效」的初步结论。
  3. **遗留交互进程要清理**：误启动的 `gh auth refresh` 设备流先 Ctrl+C 终止，
     再进入提权路径；若用户恰好已完成授权，`gh auth status` 提权后即通过。
- 沉淀建议：并入「`gh` 认证排查」经验条目（沙箱/钥匙串视图差异为第一排查项）。

## 2026-08-27 · 公开文档去「具体项目来源」要全局 sweep：引言 + 专门来源节 + 本地路径一并清理

- 来源项目/任务：通用项目模板工作区（根 README 重写）。
- 背景与上下文：用户要求重写 README，「不要提及具体项目参考来源、措辞通俗易懂、
  面向用户」。原 README 的来源引用分散两处：引言一句话（「吸收了 PinNotes 与
  KnowOps 两个项目的实战经验」）与文末「经验来源」专节（含本地绝对路径
  `<本机路径>\...`）。
- 关键方法/经验：
  1. **去引用要全局 sweep，不能只删专节**：具体项目来源常同时出现在引言、正文
     与文末专节；专节删除后必须再 grep 项目名与本地路径模式
     （`PinNotes|KnowOps|D:\|经验来源`）确认零残留。
  2. **公开 README 不该有本地绝对路径**：工作区本地路径属于机器专属信息，随仓库
     发布到 GitHub 会泄露目录结构、且对使用者无意义；面向用户的 README 只保留
     通用占位符与示例路径。
  3. **重写时保持硬事实准确**：版本号、脚本路径、命令示例等按仓库实况核对
     （如 `trash.py` 实际在 `project-template/scripts/`），避免通俗化过程中失真。
- 沉淀建议：可并入「公开文档治理」经验条目（本地路径/来源引用清理模式）。

## 2026-08-27 · `gh` CLI 令牌与 git push 凭据相互独立：push 成功 ≠ Release 可建

- 来源项目/任务：通用项目模板工作区（v1.4.2.patch0 远端发布）。
- 背景与上下文：配置远端后执行 push + Release。`git push origin main` 与
  `git push origin v1.4.2.patch0` 均成功（走系统凭据管理器），但 `gh release
  create` 返回 401——`gh auth status` 显示默认令牌已失效（The-Daybreaker）。
- 关键方法/经验：
  1. **凭据通道独立**：git HTTPS push 用系统凭据管理器，`gh` CLI 用自身 OAuth
     令牌；前者成功不能推断后者可用，反之亦然。发布流程应把 `gh auth status`
     放进推送前检查，避免「代码与 tag 已公开、Release 卡壳」的中间态。
  2. **发布中间态可观测**：推送成功后用 `git ls-remote` 核实远端 ref
     （分支 + tag 的提交号与本地一致）再落盘，避免凭「推送退出码 0」下结论；
     沙箱内 `ls-remote` 可能因凭据存储不可达报 schannel 错误，升级权限后即通。
  3. **gh 令牌失效的处置**：按模板发布流程提示用户 `gh auth login -h
     github.com`，或改网页手动创建 Release；不要把失效令牌当作临时故障反复重试。
- 沉淀建议：可并入「远端发布流程」经验条目，作为发布前置检查项。

## 2026-08-27 · 沙箱只读 `.git` 下配置远端需升级权限；维护类任务同样走「STATUS 双收口 + 自动沉淀」纪律

- 来源项目/任务：通用项目模板工作区（配置远端仓库 `origin` 维护任务）。
- 背景与上下文：用户要求为工作区配置 GitHub 远端
  （`https://github.com/The-Daybreaker/Project-Template`）。沙箱权限配置文件系统时
  `.git` 为只读，`git remote add` 首跑报 `could not lock config file .git/config:
  Permission denied`；升级权限后成功。
- 关键方法/经验：
  1. **沙箱写边界细分到 `.git`**：工作区文件可写不代表 `.git/config` 可写；
     任何 git 写操作（remote/config/commit/tag）在受限沙箱下首跑大概率失败，
     直接带 `require_escalated` 重跑即可，不必先报错再升级（一次到位更高效）。
  2. **贯穿/维护类任务也遵守状态纪律**：配置远端虽不占 P1-P5 阶段，仍按
     维护约定 #6/#7 走完「任务开始更新 STATUS 当前任务 → 完成覆盖更新 + 提交 +
     自动沉淀」闭环；STATUS 的「当前阶段」标注为贯穿动作、不虚构阶段卡。
  3. **远端配置的验收面**：`git remote add` 成功后必须 `git remote -v` 回读
     fetch/push 两条 URL 确认，避免仅凭命令退出码 0 下结论。
- 沉淀建议：可并入「状态文档生命周期开始/收尾双收口」条目，作为小任务变体。

## 2026-08-27 · 格式类红线的「断言面/表述面」失同步根因与机制化杜绝（smoke_init 冒烟代码化 + 断言面联动核对）

- 来源项目/任务：通用项目模板工作区（第九轮审计修复轮，用户要求「找根因 +
  杜绝再发」）。
- 背景与上下文：F1（新项目首跑即红）修复后追问根因。取证链：`git log -S`
  证实 `check_dev_docs.py` 的字面断言引入于 v1.4.0（当时骨架有同名独立小节，
  断言成立）；v1.4.1 阶段卡合并改造删掉该字面串但断言未联动；且修复后在
  模板目录实跑该检查仍红——反证发布时「全链验证绿」未在真实新初始化环境实跑。
- 关键方法/经验：
  1. **结构性根因模型**：格式类红线的传播面分「定义面」（权威定义文档，人工
     注意力覆盖）与「断言/表述面」（检查脚本字面断言、脚本输出文案、校验清单、
     各级 README/SKILL/DESIGN/LOADING 摘要）；后者无自动化覆盖、人工也不枚举，
     是失同步高发区。修复轮按此模型全工作区 sweep，在审计认定的 3 处之外再
     发现 6 处残留——模型正确性当场实证。
  2. **机制化杜绝（三件，均已实施）**：① 冒烟深度代码化——
     `scripts/smoke_init.py` 把「初始化+回读+冒烟项目内骨架脚本自检四连」固化
     为可重复脚本，列入维护约定 #10 发版前必跑项（验证环节不依赖记忆与诚实，
     依赖退出码）；② 断言面联动核对写进维护约定——改格式/字段类红线必须全量
     grep 枚举断言面并零残留；③ 工具自身的同源缺陷模式排查——sync 的清理/复制
     共用跳过列表导致「排除项进镜像即永存」，提示：跳过/排除逻辑的清理路径
     与生产路径必须分开审视。
  3. **审计自身也吃这个亏**：首轮审计用定点文件 grep 只列 3 处残留；修复轮
     全工作区 sweep 才见全貌。教训：审计「残留类」发现必须全工作区模式 grep，
     不允许定点枚举。
  4. **发布验证的可信度检查**：声称「验证绿」的项要能当场重放；重放不出绿
     （如模板目录实跑即红）说明发布时该验证未真实发生——这是比缺陷本身更
     严重的流程信号。
- 验证/效果：修复后 `smoke_init.py` 全项 PASS（含冒烟项目四连绿）；六处副本
  重装 + sync 全绿；旧表述终扫零残留；机制随本轮复检完整走通一次。
- 相关文件：`scripts/smoke_init.py`、`AGENTS.md`（维护约定 #10）、
  `README.md`（维护约定）、`scripts/sync_template.py`（排除项清理）、
  `docs/AUDIT-2026-08-27.md`（§六）
- 建议 KB 属性（沉淀时参考，可调整）：`type=knowledge`；
  `knowledge_type=经验方法/方案`；`domain=项目管理/质量审计`；
  `tags=[根因分析, 防再发机制, 冒烟测试, 文档治理]`；`project=通用项目模板`
- 状态：待沉淀
- 沉淀日期：

## 2026-08-27 · 开箱即用审计要穿透到「骨架脚本自检」：第九轮全链实测发现新项目首跑即红（字面断言与合并阶段卡骨架失同步）

- 来源项目/任务：通用项目模板工作区（第九轮全面审计，实测优先）。
- 背景与上下文：前八轮审计的「初始化冒烟」只验证到 init 脚本退出码 0 +
  占位符无残留。本轮把冒烟穿透到「冒烟项目内实跑骨架脚本四连」
  （ci_check / check_dev_docs / trash --help / pre_release_check），
  立刻发现新项目首跑即红：`check_dev_docs.py` 以字面串断言
  「生命周期合规清单」，而 v1.4.1.patch0 阶段卡合并改造后 STATUS 骨架
  已无该字面串（改为「合规：✓/⏳」两行式）——脚本自述「freshly
  initialized projects are fine」与实测直接冲突。同轮还发现旧版阶段卡
  表述残留在 `init_project.py` 输出与 `init-steps.md` 两处（语义级过时，
  自动化覆盖校验测不到）。
- 关键方法/经验：
  1. **冒烟深度标准**：「初始化成功 ≠ 开箱即用」。冒烟必须包含冒烟项目内
     骨架脚本自检（至少 ci_check + check_dev_docs 退出码 0），并写入
     审计清单与 init-steps，形成固定动作。
  2. **字面断言是格式红线的影子**：任何展示格式变更（阶段卡字段增删）都
     必须联动检查所有「字面字符串断言 / 示例文本 / 输出文案」——本轮
     失同步三处（检查脚本断言、init 输出提示、落地路线图）全部是字面
     层面的残留；发版清单应显式列「字面断言核对项」。
  3. **自动化校验的边界自觉**：`INIT_STEPS_COVERAGE`、继承矩阵指纹等
     只能证「文件级覆盖/正文指纹」，证不了「语义级过时」；这类盲区靠
     审计实测兜底，不能指望加一条静态校验就一劳永逸。
  4. 全链实测顺序（可复用）：同步链 → 副本哈希 → skill 校验 → py_compile
     → 初始化冒烟 → **冒烟项目脚本自检** → 版本/密钥/行尾全局扫描 →
     文档硬事实 grep。
- 验证/效果：冒烟项目实测 `check_dev_docs` 退出码 1（证据链见
  `docs/AUDIT-2026-08-27.md`）；同轮其余 16 项实测全绿（版本链 9 处一致、
  tag==HEAD、密钥阴性、红线恰 17 条、六副本哈希一致等）。
- 相关文件：`project-template/scripts/check_dev_docs.py`、
  `project-template/private/dev/STATUS.md`（骨架）、
  `skills/init-project/scripts/init_project.py`、
  `skills/init-project/references/init-steps.md`、`docs/FLOW.md`（工作区）、
  `docs/AUDIT-2026-08-27.md`（本报告）
- 建议 KB 属性（沉淀时参考，可调整）：`type=knowledge`；
  `knowledge_type=经验方法/教训`；`domain=项目管理/质量审计`；
  `tags=[审计, 冒烟测试, 文档治理, 模板工程]`；`project=通用项目模板`
- 状态：待沉淀
- 沉淀日期：

## 2026-08-27 · 阶段卡反定型为条件块（仅关键/风险节点展示）+ 母项目本地发版 v1.4.1（无远端）

- 来源项目/任务：通用项目模板工作区（v1.4.1.patch0 发版前修正）。
- 背景与上下文：阶段卡合并模块初版把反定型当作固定块；用户指出反定型是**条件块**——
  仅关键/风险节点（确认方案类节点）展示，收尾/进度展示节点不应带上。本次修正模板
  PHASES §5 / STATUS 骨架 / AGENTS / audit-checklist / agent-rules 同步，并在收尾
  汇报中省略反定型块（dogfood）。
- 关键方法/经验：
  1. 反定型定位：红线 17 的展示动作 = 关键/风险节点的必查项，非阶段卡固定字段；
     固定块 = 标题 + 阶段线 + 合规两行，反定型 = 条件块。
  2. 发版教训：母项目发版**不用** bump_version.py（它会误升模板骨架 version），按
     发版同步约定手工改（根 version.json / 模板 template_version / 两 skill
     metadata / 继承矩阵版本对照 / 当前版本字样 / CHANGELOG 归档 / UPGRADE 迁移要点）
     + 全局 grep 新旧版本号（残留=历史引用/规则示例可保留）。
  3. 本仓库无 git remote：发布只能到「本地版本同步 + 本地 tag」；push/Release 需
     用户先配置远端（gh 可用但无 repo）。
- 沉淀建议：可进知识库「AI 协作/发布流程」类目；模板侧已直接落地。

## 2026-08-27 · 阶段卡/生命周期合规清单/反定型合并紧凑模块：横置阶段线 + 合规两行 + 反定型紧凑内容 + 全中文展示

- 来源项目/任务：通用项目模板工作区（用户对阶段卡展示的可用性改造）。
- 背景与上下文：原阶段卡为表格（模块·子阶段/正在完成/已完成/下一步/状态）+ 独立
  合规清单表 + 反定型大段；用户反馈：字母缩写理解压力大、篇幅大，希望合并紧凑、
  顶部横置阶段线标注当前位置、展示只用中文。
- 关键方法/经验：
  1. 合并模块设计：标题（含状态）→ 横置阶段线（标签=阶段中文名、当前节点加粗、
     只显示当前阶段节点）→ 合规两行（✓ 已完成 / ⏳ 待完成，分号分隔）→ 反定型
     6 项紧凑内容；去掉「正在完成/已完成/下一步」字段（进度细节归 STATUS
     「当前任务」）。
  2. 可读性：阶段卡内只用中文名称、不显示字母缩写（保留数字编号）；「缩写附中文
     翻译」规则仍适用于阶段卡之外。
  3. 用户协作方式：以原型预览 + 注释逐条修订（去三行/阶段名作标签/状态入标题括号/
     合规两行结构化/反定型保留内容）迭代收敛，比一次性大改更稳。
  4. 流程：模板【通用】定义（PHASES §5）+ STATUS 骨架 + AGENTS 展示规则 + 审计清单
     + agent-rules 精简版同步 + 资产镜像 + 六处副本重装 + sync 全链绿。
- 沉淀建议：可进知识库「AI 协作/文档设计」类目；模板侧已直接落地。

## 2026-08-27 · agent 提问与共识确认机制（红线 17）：回答≠确认、禁面板、共识快照+逐项表态、反定型 6 项关键节点展示

- 来源项目/任务：通用项目模板工作区（用户提出「被 agent 问题推着走」的痛点，
  重新设计提问与共识确认机制；PRD-0001/RESEARCH-0001/ADR-0001 落于 `docs/`）。
- 背景与上下文：主流 agent 用选择题面板收集需求，用户作答被当作确认，agent 自设
  问题切面当需求；用户要求：禁面板（聊天内可给选项/推荐但不得仅依赖选项推进）、
  回答=新信息输入（每次回答后内部重检问题空间）、共识快照+逐项表态（推进门槛）、
  反定型 6 项（当前理解/我的假设/尚未确认/反例/替代解释/重新审视问题）仅关键/风险
  节点展示（平时内部思考）、确认不锁定（需求阶段内新信息可回审，PRD 定稿后走
  开新 PRD 取代）。
- 关键方法/经验：
  1. 调研（红线 13）确认：面板是主流默认，但疲劳/诱导副作用有广泛讨论；「总结+
     最终确认」（SpecBench）、「假设显式化」（discover）、「显式确认后推进」
     （clarify-first/Tessl）为散点实践，本机制属组合创新——机制类改动先调研可避免
     重复造轮子，也避免「现成同款」误判。
  2. 落地要点：红线进模板【通用】+ agent-rules 精简版（继承矩阵指纹自动校验正文
     变化）；PHASES 需求引导方法论 + FLOW 状态机 + audit-checklist 检查项同步；
     共识快照与阶段卡整合（不新增独立仪式）；变更流程复用 PRD 取代规则（轻量，
     不另建机制）。
  3. 展示克制：反定型 6 项不必每次展示（会形式主义），关键/风险节点展示、平时
     内部思考——机制设计要防「新仪式」代替旧问题。
  4. 流程教训：轻量项目按用户指示裁剪（无 DESIGN 文件直接进实现），但 PRD/ADR/
     调研记录仍保留（决策可追溯）；委派子代理须硬性声明「只读/禁写」，且提交前
     回读核实提交内容（子代理曾误把审计任务当收口执行并自行提交）。
- 沉淀建议：可进知识库「AI 协作/需求工程」类目；模板侧已直接落地（红线 17 +
  机制文档），无需 EXPERIENCE-TO-TEMPLATE 暂存。
...
- 来源项目/任务：通用项目模板工作区全面专业审计（五视角：架构/全栈/agent/审计/用户）
- 背景与上下文：对当前项目做全量审计，复验全链自动化（sync / verify /
  check_dev_docs / quick_validate / py_compile）+ 端到端冒烟，并逐条实测文档中的
  可执行命令。
- 需求/问题：审计要发现「文档命令照抄必失败」与「状态文档自身元数据过期」两类
  隐蔽缺陷——自动化全绿时它们仍可能长期存在。
- 做法与过程：
  1. 实测文档命令：quick_validate 文档写 `quick_validate.py init-project`，实际
     脚本要求 `<skill_directory>` 目录路径，照抄必报 `SKILL.md not found`
     （存在多轮未被发现——audit-checklist §7「指引可执行」应实测命令而非只读文档）；
  2. 状态文档元数据 vs git 交叉核对：STATUS「最后更新 19:13 / 提交 94c8486」与
     实际 HEAD（4cb4450 @ 19:20）不一致；EXP-KB 最后更新时间戳早于其最后改动提交；
     CHANGELOG 缺「未发版变更」区段（v1.4.0 后有实质提交未记录）——均为「收尾双
     收口」与「未发版区段也是状态文档」两条既有教训的复发；
  3. 修复落地时发现：改 skill 承载文档（init-steps.md / agents/openai.yaml）后，
     verify_installed_copies 会拦截「六处安装副本未重装」——强制机制生效；
  4. 收尾清理：`_trash/` 遗留 3 轮 trae 临时删除区按删除纪律整体进回收站（可恢复）。
- 经验/教训：
  - 「照文档执行」是审计清单里最便宜也最有效的检查：对文档中的每条命令/路径直接
    实跑一遍，能发现多轮自动化全绿下的文档级缺陷；
  - 状态文档的「最后更新 / 提交号」属于硬事实，收尾时必须以 `git log` 实际校准
    （先核对 git 再写文档），且未发版区段不写死 HEAD/提交号（以提交记录为准）；
  - skill 承载文档（references/init-steps.md、agents/*.yaml）与六处安装副本属于
    同一个「分发面」：改动后必须重装 + 哈希复核，否则 sync/verify 拦截；
  - 文档树/清单类内容（README 结构树、init-steps 校验清单）会随功能演进漏更，
    审计时把「文档里写到的文件/参数/命令」与仓库实际清单逐一比对。
- 验证/效果：P1×1 + P2×3 + P3×4 全部修复落地；重装六处副本后全链验证绿
  （sync / verify / check_dev_docs / quick_validate×2 / py_compile）；冒烟初始化
  全链通过。
- 相关文件：`AGENTS.md`、`README.md`、`docs/{STATUS,CHANGELOG,EXPERIENCE-TO-KB}.md`、
  `skills/init-project/{references/init-steps.md,agents/openai.yaml}`、
  `scripts/{sync_template.py,verify_installed_copies.py}`
- 建议 KB 属性（沉淀时参考，可调整）：`type=knowledge`；
  `knowledge_type=经验方法/教训`；`domain=文档治理/Agent 工程实践/审计`；
  `tags=[审计, 文档可执行性, 状态文档, 硬事实校准, skill 分发]`；`project=通用项目模板`
- 状态：待沉淀
- 沉淀日期：
