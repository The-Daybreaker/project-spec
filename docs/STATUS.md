...
14 tag 全重映射）；旧对象物理清除（fsck 空）；审计报告邮箱字面残留清除 +
脱敏前历史 blob 字面内容级清除（局部 tree-filter）；扫描器 noreply 白名单；
发现并修复重写后新增 3 提交的旧身份再污染（本地 `user.email` 配置同步）；
强推 main + 全部 tag；远端引用全量对照一致；Release 指针核实；全历史终门禁绿
（`--history --strict`：0 高危 / 0 个人信息）
