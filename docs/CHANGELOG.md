...
- **阶段卡/生命周期合规清单/反定型合并模块重设计**：阶段卡改为合并紧凑模块——标题
  ...
  当前阶段节点）、合规两行（✓ 已完成 / ⏳ 待完成，分号分隔）、反定型紧凑内容（6 项
  附内容非仅勾选）；去掉「正在完成/已完成/下一步」字段；**阶段卡展示只用中文名称、
  不显示字母缩写**（保留数字编号）；PHASES §5 定义 / STATUS 骨架 / AGENTS 展示规则 /
  audit-checklist / agent-rules 精简版同步。
